...
public class StatusUtil {
    ...
    public static Status getStatus(@NonNull DownloadTask task) {
        final Status status = isCompleteOrUnKnown(task);
    ...
    public static Status isCompleteOrUnKnown(@NonNull DownloadTask task) {
        ...
        if (info != null) {
            if (!info.isChunked() && info.getTotalLength() <= 0) {
                ...
                && targetFile.exists() && info.getTotalOffset() == info.getTotalLength()) {
                return Status.COMPLETED;
            } else if (filename == null && info.getFile() != null && info.getFile().exists()) {
                return Status.IDLE;
            } else if (targetFile != null && targetFile.equals(info.getFile()) && targetFile.exists()) {
                return Status.IDLE;
            }
