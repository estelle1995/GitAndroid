...
import com.example.myokdownload.dowload.core.listener.DownloadListener2;
import com.example.myokdownload.dowload.core.listener.DownloadListenerBunch;
...
public class DownloadSerialQueue extends DownloadListener2 implements Runnable {
    private static final Executor SERIAL_EXECUTOR = new ThreadPoolExecutor(0,
            ...
            ThreadUtil.threadFactory("OkDownload DynamicSerial", false));
    ...
    public synchronized void pause() {
        if (paused) {
            LogUtil.w(TAG, "require pause this queue(remain " + taskList.size() + "), but"
    ...
    public synchronized void resume() {
        if (!paused) {
            LogUtil.w(TAG, "require resume this queue(remain " + taskList.size() + "), but it is"
                    + " still running");
            return;
        }
        paused = false;
